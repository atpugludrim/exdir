...
class partial_callable_get_MPTree:
    def __init__(this, G, n_hops, func):
        this.G = G
        this.n_hops = n_hops
        this.func = func

    def __call__(this, nodes):
        return this.func(this.G, nodes, this.n_hops)


def generate_trees(dataset, n_hops=3):
    # create two (multivalued) functions:
    # f1 : graph -> trees
    # f2 : tree -> graphs
    # since these are finite functions, they will be implemented as dictionaries
    f1 = {}
    f2 = {}
    # also create one more function that maps class to graphs:
    # f3 : class -> graphs (again, dictionary)
    f3 = {}
    # I'll also create a tree -> class(es) map:
    # f4 : tree -> classes
    f4 = {}
    canlabs = {}
    for idx, data in enumerate(dataset):
        ...
        G = to_nx(data)
        G = get_labels(G)
        partial_func = partial_callable_get_MPTree(G, n_hops, get_MPTree)
        with Pool(processes=32) as pool:
            pool_iter = pool.imap_unordered(partial_func, G.nodes)
            for MPTree in pool_iter:
